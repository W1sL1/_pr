...
#include <stdio.h>  
#include <stdlib.h> // Библиотека для работы с памятью (realloc, free)

int main() {
    int* a = NULL;  // Указатель для динамического массива коэффициентов
    int count = 0;  // Счетчик количества введенных чисел
    int num;        // Буфер для текущего считанного числа

    // Цикл работает, пока пользователь вводит целые числа
    while (scanf("%d", &num) == 1) {
        // Увеличиваем массив на 1 элемент при каждом вводе
        a = (int*)realloc(a, (count + 1) * sizeof(int));
        
        // Если память не выделилась, аварийно завершаем программу
        if (a == NULL) return 1; 
        
        // Записываем число в массив и увеличиваем счетчик
        a[count++] = num;
    }
    // Если ввели меньше 2 чисел (нужны хотя бы коэффициенты и N), выходим
    if (count < 2) { free(a); return 1; }

    // Последнее число в вводе — это N (до какого индекса считать дробь)
    int n = a[count - 1];
    
    // Уменьшаем count, чтобы он указывал только на количество коэффициентов
    count--; 

    // Если N больше, чем количество имеющихся коэффициентов, выходим
    if (n >= count) { free(a); return 1; }

    // Начальные значения для итерации (база для индекса -1 и 0)
    long long p_prev2 = 1, p_prev1 = a[0]; // P для индексов -2 и -1 (условно)
    long long q_prev2 = 0, q_prev1 = 1;    // Q для индексов -2 и -1
    
    // Результат для случая n=0 (просто первый коэффициент a[0]/1)
    long long p_curr = p_prev1, q_curr = q_prev1;

    // Цикл расчета со 2-го элемента (индекс 1) до n-го
    for (int i = 1; i <= n; i++) {
        // Основная формула цепной дроби
        p_curr = a[i] * p_prev1 + p_prev2;
        q_curr = a[i] * q_prev1 + q_prev2;

        // Сдвигаем значения для следующего шага (предыдущее становится пред-предыдущим)
        p_prev2 = p_prev1; q_prev2 = q_prev1;
        p_prev1 = p_curr;  q_prev1 = q_curr;
    }
    // Выводим итоговый числитель и знаменатель
    printf("%lld %lld\n", p_curr, q_curr);

    // Освобождаем динамическую память, чтобы не было утечек
    free(a); 
    return 0;
}
