...
int main(void) {
    size_t n_arrays;
    if (scanf("%zu", &n_arrays) != 1) return 0;

    int *result = NULL;      // итоговый массив
    size_t result_len = 0;   // текущая длина result

    for (size_t t = 0; t < n_arrays; ++t) {
        size_t m;
        if (scanf("%zu", &m) != 1) return 0;

        // 1) динамическая память ровно под m элементов для текущего входного массива
        int *arr = NULL;
        if (m > 0) {
        ...
        // 2) объединяем: если это первый массив — забираем его память
        if (result_len == 0) {
            result = arr;
            result_len = m;
            arr = NULL; // теперь result владеет памятью
        } else {
            // 3) слияние result (отсорт.) и arr (отсорт.) => merged (точный размер)
            size_t new_len = result_len + m;
            int *merged = NULL;
            if (new_len > 0) {
                merged = (int*)malloc(new_len * sizeof(int));
                if (!merged) {
                    fprintf(stderr, "Memory allocation failed\n");
                    free(arr);
                    free(result);
            ...
            merge_sorted(result, result_len, arr, m, merged);
    ...
    // 5) освобождение всей динамической памяти
    free(result);
    return 0;
}
