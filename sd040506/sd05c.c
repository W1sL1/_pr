#include <stdio.h>   // printf, scanf, fopen, fprintf, fgets, perror
#include <stdlib.h>  // malloc, free
#include <string.h>  // strcmp, strstr, strcspn
#include <time.h>    // clock_gettime, timespec

#define DESC_LEN 256   // максимальная длина описания события (+ завершающий '\0' в буфере)
#define PLACE_LEN 128  // максимальная длина места (+ '\0')

// Структура события
typedef struct Event {
    int year, month, day, hour, minute; // календарная дата и время суток
...
// Узел AVL-дерева (сбалансированное бинарное дерево поиска)
typedef struct Node {
    Event event;           // ключ сортировки — поля внутри Event (см. compare_events)
    struct Node *left;     // левое поддерево (меньше по compare_events)
    struct Node *right;    // правое поддерево (больше или равно)
    int height;            // высота узла для балансировки AVL
} Node;

// Прототипы функций
int is_valid_date(int year, int month, int day);                                     // проверка даты
int is_valid_time(int hour, int minute);                                           // проверка времени
int calc_weekday(int year, int month, int day);                                    // день недели (Пн=0)
Node* add_event(Node* root, Event event);                                           // вставка в AVL
Node* delete_event(Node* root, Event event);                                       // удаление из AVL
void inorder(Node* root);                                                           // обход слева-корень-справа
void filter_by_place(Node* root, const char* place_substr);                         // подстрока в place
void search_by_datetime(Node* root, int year, int month, int day, int hour, int minute, int *found); // точное совпадение даты/времени
void save_events(Node* root, const char* filename);                                 // запись дерева в файл
Node* load_events(const char* filename);                                           // чтение файла и построение дерева
void free_tree(Node* root);                                                        // рекурсивное освобождение узлов
int compare_events(Event e1, Event e2);                                             // полное сравнение событий
void print_event(Event event);                                                     // печать одной записи

// --- AVL tree helpers ---

int node_height(Node* n) {
    return n ? n->height : 0; // у пустого узла высота 0
}

int max(int a, int b) {
    return a > b ? a : b; // для пересчёта height = 1 + max(...)
}

// Получить баланс-фактор (разницу высот левого и правого поддерева)
int get_balance(Node* n) {
    if (!n) return 0; // у NULL баланс не определён — считаем 0
    return node_height(n->left) - node_height(n->right);
}

// Правое вращение
Node* right_rotate(Node* y) {
    Node* x = y->left;   // новый корень поддерева после поворота
    Node* T2 = x->right; // поддерево «между» x и y переподвешиваем
    ...
    // Обновляем высоты
    y->height = 1 + max(node_height(y->left), node_height(y->right)); // сначала нижний бывший корень
...
Node* left_rotate(Node* x) {
    Node* y = x->right;  // новый корень поддерева
    Node* T2 = y->left;  // среднее поддерево
...
void write_event_text(FILE* f, Event* e) {
    ...
    }; // индекс совпадает с полем weekday (0 = Monday в коде)
    ...
    fprintf(f, "%04d-%02d-%02d %02d:%02d (%s) | Place: %s | %s\n",
        ...
        weekday_names[e->weekday], e->place, e->description // одна строка на событие
    );
}

// Функция для чтения одного Event из текстового файла
int read_event_text(FILE* f, Event* e) {
    char buf[DESC_LEN + PLACE_LEN + 128]; // вся строка файла + запас под формат
    char weekday_str[16]; // английское имя дня из скобок
    ...
    int parsed = sscanf(buf, "%d-%d-%d %d:%d (%15[^)]) | Place: %127[^|]| %255[^\n]",
        &e->year, &e->month, &e->day, &e->hour, &e->minute,
        weekday_str, e->place, e->description); // ожидаем 8 успешных полей
    ...
    return 1; // событие успешно разобрано
}

int main() {
    Node* root = NULL; // корень BST/AVL (изначально пустое дерево)
    int choice;
    while (1) { // главный цикл меню до выхода (пункт 0)
        // Меню пользователя
        printf("\nMenu:\n");
        printf("1. Add event\n");
        ...
        if (scanf("%d", &choice) != 1) {
            ...
            while (getchar() != '\n'); // сброс до конца строки при нечисловом вводе
        ...
        if (choice == 1) { // Добавить событие
            ...
            root = add_event(root, event); // вставка с балансировкой
            printf("Event added!\n");
            ...
            printf("Hour: "); scanf("%d", &event.hour);
            printf("Minute: "); scanf("%d", &event.minute);
            event.weekday = calc_weekday(event.year, event.month, event.day); // полное совпадение при удалении
            ...
            fgets(event.place, PLACE_LEN, stdin);
            event.place[strcspn(event.place, "\n")] = 0;
            root = delete_event(root, event); // удаление и перебалансировка
            ...
            place[strcspn(place, "\n")] = 0;
            filter_by_place(root, place); // in-order обход с strstr
        } else if (choice == 5) {
            int y, m, d, h, min, found = 0; // found — флаг «нашли хотя бы одно»
            ...
            search_by_datetime(root, y, m, d, h, min, &found); // полный обход in-order

            if (clock_gettime(CLOCK_MONOTONIC, &end) != 0) {
                perror("clock_gettime");
            }

            elapsed = (end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec) / 1000000000.0; // секунды с дробной частью
            ...
            fname[strcspn(fname, "\n")] = 0;
            save_events(root, fname); // перезапись файла содержимым дерева
        } else if (choice == 7) {
            char fname[128];
            getchar(); // Считываем перевод строки
            printf("Enter file name: ");
            fgets(fname, 128, stdin);
            fname[strcspn(fname, "\n")] = 0;
            free_tree(root); // старое дерево перед загрузкой
            root = load_events(fname); // новое дерево только из файла
        } else {
...
int is_valid_date(int year, int month, int day) {
    if (year < 1900 || year > 2100) return 0; // ограничение задачи
    if (month < 1 || month > 12) return 0;
    int days_in_month[] = { 31,28,31,30,31,30,31,31,30,31,30,31 }; // февраль поправим ниже
    // Проверка на високосный год
    if ( (year%4==0&&year%100!=0)||year%400==0 ) days_in_month[1]=29;
    if (day<1 || day>days_in_month[month-1]) return 0; // день в пределах месяца
    return 1;
}

// Проверка корректности времени
int is_valid_time(int hour, int minute) {
    return (hour >= 0 && hour < 24 && minute >= 0 && minute < 60); // сутки и минуты в минуте
}
...
int calc_weekday(int year, int month, int day) {
    ...
    if (month < 3) { month += 12; year--; } // январь/февраль считаются как 13/14 предыдущего года
    int K = year % 100; // год внутри века
...
int compare_events(Event e1, Event e2) {
    ...
    return strcmp(e1.place, e2.place); // полное лексикографическое упорядочивание
}

// Добавить событие в сбалансированное АVL дерево поиска
Node* add_event(Node* root, Event event) {
    ...
    if (compare_events(event, root->event) < 0) {
        root->left = add_event(root->left, event); // идём влево если «меньше»
    } else {
        root->right = add_event(root->right, event); // дубликаты по правилам — вправо
    }
    ...
    // Left Left
    if (balance > 1 && compare_events(event, root->left->event) < 0)
        return right_rotate(root);

    // Right Right
    if (balance < -1 && compare_events(event, root->right->event) > 0)
        return left_rotate(root);

    // Left Right
    if (balance > 1 && compare_events(event, root->left->event) > 0) {
        root->left = left_rotate(root->left); // двойной поворот
        return right_rotate(root);
    }
    ...
    return root; // дерево уже сбалансировано
}
...
Node* min_value_node(Node* node) {
    ...
    while (current && current->left != NULL)
        current = current->left; // в BST минимум — крайний левый
    return current;
}

// Удалить событие из сбалансированного дерева поиска
Node* delete_event(Node* root, Event event) {
    if (!root) return root; // нечего удалять
    ...
    if (cmp < 0) {
        ...
        // Если только один потомок или ни одного
        if (!root->left || !root->right) {
            Node* temp = root->left ? root->left : root->right; // единственный ребёнок или NULL
            if (!temp) {
                // Нет потомков
                temp = root;
                root = NULL;
                ...
                // Один потомок
                *root = *temp; // копируем данные ребёнка в текущий узел (адрес root сохраняем)
            }
            free(temp);
        } else {
            // Два потомка: берем следующий по значению элемент (наименьший справа)
            Node* temp = min_value_node(root->right);
            root->event = temp->event;
            root->right = delete_event(root->right, temp->event); // удаляем дубликат ключа справа
        }
    }

    if (!root) return root; // поддерево могло опустеть
...
void print_event(Event event) {
    ...
    printf("%04d-%02d-%02d %02d:%02d (%s) | Place: %s | %s\n",
        ...
        weekday_names[event.weekday], event.place, event.description // как в save/load
    );
}
...
void inorder(Node* root) {
    ...
    inorder(root->left);
    print_event(root->event); // возрастающий порядок ключей
    inorder(root->right);
}
...
void filter_by_place(Node* root, const char* place_substr) {
    ...
    if (strstr(root->event.place, place_substr)) // NULL если подстроки нет
...
void search_by_datetime(Node* root, int year, int month, int day, int hour, int minute, int *found) {
    ...
    {
        print_event(root->event);
        *found = 1; // хотя бы одно совпадение
    }
...
void write_node_text(FILE* f, Node* node) {
    ...
    write_event_text(f, &node->event); // один fprintf на узел
...
void save_events(Node* root, const char* filename) {
    FILE* f = fopen(filename, "w"); // перезапись с нуля
    if (!f) { 
        printf("Error opening file for writing\n"); 
        return; 
    }
    write_node_text(f, root); // in-order — отсортированный вывод
    fclose(f);
...
Node* load_events(const char* filename) {
    ...
    while (read_event_text(f, &e)) {
        root = add_event(root, e); // каждая строка — новая вставка
    }
    fclose(f);
...
void free_tree(Node* root) {
    if (!root) return;
    free_tree(root->left);  // пост-порядок: сначала дети
