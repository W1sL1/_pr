#include <stdio.h>   // ввод-вывод (printf, scanf, FILE)
#include <stdlib.h>  // malloc, free
#include <string.h>  // strcmp, strstr, strcspn, strlen
#include <time.h>    // clock_gettime, timespec

#define DESC_LEN 256   // макс. длина описания события (+ '\0')
#define PLACE_LEN 128  // макс. длина места (+ '\0')
...
// Узел дерева
typedef struct Node {
    Event event;           // полезная нагрузка узла
    struct Node *left;   // левый потомок (меньше по compare_events)
    struct Node *right;  // правый потомок (больше или равно)
} Node;

// Прототипы функций
int is_valid_date(int year, int month, int day);  // границы года/месяца/дня
int is_valid_time(int hour, int minute);          // часы и минуты в сутках
int calc_weekday(int year, int month, int day);   // Пн=0 … Вс=6
Node* add_event(Node* root, Event event);         // вставка в BST
Node* delete_event(Node* root, Event event);      // удаление по полному ключу
void inorder(Node* root);                         // симметричный обход + печать
void filter_by_place(Node* root, const char* place_substr);  // фильтр по подстроке места
void search_by_datetime(Node* root, int year, int month, int day, int hour, int minute, int *found);  // линейный поиск по дереву
void save_events(Node* root, const char* filename);   // запись дерева в файл
Node* load_events(const char* filename);             // чтение файла → новое дерево
void free_tree(Node* root);                       // рекурсивное free всех узлов
int compare_events(Event e1, Event e2);          // полный порядок для BST
void print_event(Event event);                   // одна строка в стиле файла/меню

//  Функция для записи одного Event в текстовый файл
void write_event_text(FILE* f, Event* e) {
    char *weekday_names[] = {  // англ. имена для строки в файле
...
int read_event_text(FILE* f, Event* e) {
    char buf[DESC_LEN + PLACE_LEN + 128];  // буфер целой строки записи
    char weekday_str[16];  // временно: текст дня недели из скобок

    if (!fgets(buf, sizeof(buf), f)) return 0;  // EOF или ошибка — строки нет
    ...
    int found_weekday = -1;  // -1: строка не совпала ни с одним днём
    for (int i = 0; i < 7; ++i) {
    ...
    return 1;  // успешно разобрали одну запись
}

int main() {
    Node* root = NULL;  // пустое BST в начале
    ...
    while (1) {
        ...
        if (choice == 1) { // Добавить событие
            ...
            root = add_event(root, event);  // вставка в BST по compare_events
            printf("Event added!\n");
            ...
            fgets(event.place, PLACE_LEN, stdin);
            event.place[strcspn(event.place, "\n")] = 0;
            root = delete_event(root, event);  // удалить совпадающий ключ (если есть)
            ...
            place[strcspn(place, "\n")] = 0;
            filter_by_place(root, place);  // обход с strstr по полю place
        } else if (choice == 5) {
            int y, m, d, h, min, found = 0;  // found: нашли ли хотя бы одно совпадение
            struct timespec begin, end;     // монотонные метки времени
            double elapsed;
            ...
            search_by_datetime(root, y, m, d, h, min, &found);  // полный обход дерева
            ...
            fname[strcspn(fname, "\n")] = 0;
            save_events(root, fname);  // in-order запись в текстовый файл
        } else if (choice == 7) {
            char fname[128];
            getchar(); // Считываем перевод строки
            printf("Enter file name: ");
            fgets(fname, 128, stdin);
            fname[strcspn(fname, "\n")] = 0;
            free_tree(root);           // старое дерево уничтожаем
            root = load_events(fname);  // строим новое из файла
        } else {
...
int is_valid_date(int year, int month, int day) {
    if (year < 1900 || year > 2100) return 0;  // допустимый диапазон года
    if (month < 1 || month > 12) return 0;
    int days_in_month[] = { 31,28,31,30,31,30,31,31,30,31,30,31 };  // февраль пока 28
    // Проверка на високосный год
    if ( (year%4==0&&year%100!=0)||year%400==0 ) days_in_month[1]=29;  // февраль = 29
    if (day<1 || day>days_in_month[month-1]) return 0;  // день вне месяца
    return 1;
}

// Проверка корректности времени
int is_valid_time(int hour, int minute) {
    return (hour >= 0 && hour < 24 && minute >= 0 && minute < 60);  // сутки и минуты
}
...
int calc_weekday(int year, int month, int day) {
    ...
    if (month < 3) { month += 12; year--; }  // январь/февраль считаем как 13/14 прошлого года
...
int compare_events(Event e1, Event e2) {
    ...
    int cmpd = strcmp(e1.description, e2.description);  // разрыв ничьей по дате/времени
    if (cmpd != 0) return cmpd;
    return strcmp(e1.place, e2.place);  // последний критерий полного порядка
}

// Добавить событие в бинарное дерево поиска
Node* add_event(Node* root, Event event) {
    if (root == NULL) {
        Node* n = (Node*)malloc(sizeof(Node));  // лист: новый узел
        n->event = event;
        n->left = n->right = NULL;
        return n;
    }
    if (compare_events(event, root->event) < 0) {
        root->left = add_event(root->left, event);  // меньше — влево
    } else {
        root->right = add_event(root->right, event);  // не меньше — вправо (дубликаты справа)
    }
    return root;  // корень поддерева не меняется при рекурсии снизу
}
...
Node* min_value_node(Node* node) {
    ...
    while (current && current->left != NULL)
        current = current->left;  // идём влево до упора
    return current;
}
...
Node* delete_event(Node* root, Event event) {
    ...
    int cmp = compare_events(event, root->event);  // куда спускаться
    if (cmp < 0) {
        ...
        if (!root->left) {
            Node* temp = root->right;  // замена: только правый ребёнок
            free(root);
            return temp;
        } else if (!root->right) {
            Node* temp = root->left;
        ...
        Node* temp = min_value_node(root->right);  // inorder-преемник в правом поддереве
        root->event = temp->event;  // копируем данные преемника
...
void inorder(Node* root) {
    ...
    inorder(root->left);           // сначала меньшие
    print_event(root->event);     // текущий узел
    inorder(root->right);         // затем большие
}
...
void filter_by_place(Node* root, const char* place_substr) {
    ...
    if (strstr(root->event.place, place_substr))  // подстрока в place
...
void search_by_datetime(Node* root, int year, int month, int day, int hour, int minute, int *found) {
    ...
    {
        print_event(root->event);
        *found = 1;  // пометить успех (может быть несколько — всё равно 1)
    }
...
void write_node_text(FILE* f, Node* node) {
    ...
    write_event_text(f, &node->event);  // одна строка файла на событие
...
void save_events(Node* root, const char* filename) {
    FILE* f = fopen(filename, "w");  // перезапись файла
    if (!f) { 
...
Node* load_events(const char* filename) {
    FILE* f = fopen(filename, "r");  // только чтение
    if (!f) { 
        printf("Error opening file for reading\n"); 
        return NULL; 
    }
    Node* root = NULL;  // новое дерево с нуля
    Event e;
    while (read_event_text(f, &e)) {  // пока в файле есть валидные строки
