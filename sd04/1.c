...
void add_to_Date_tree(Node item) {
	if (Date_Root == NULL) {
		...
		Node* p = Date_Root;
		long long event_time_item = ((item.year - 1970) * 525600) + item.mon * 43800 + item.day * 1400 + item.hour * 60 + item.min - 27180; // в конце поправка на високосный 
		while (1) {
			long long event_time_p = ((p->year - 1970) * 525600) + p->mon * 43800 + p->day * 1400 + p->hour * 60 + p->min - 27180; // в конце поправка для корректности
...
void Traverse(Node *p) {
	...
	printf("%d:", p->hour);
	if (p->min < 10) printf("0"); // чтобы условно 13:01 не выглядело как 13:1
	printf("%d\n", p->min);
...
int if_data_correct(Node item) {
	...
	event_time = ((item.year - 1970) * 525600 + item.mon * 43800 + item.day * 1400 + item.hour * 60 + item.min) - 27180; // в конце поправка
...
Node ask_for_new_object() {
	...
	do {
		...
		char aaaa[213];  
		fgets(aaaa, sizeof(aaaa), stdin); // fgets забирает какой-то символ \n
...
Node* search_by_time(Node item, Node* p, int* left_or_right) {
	Node* parent = NULL;
	long long event_time_item = 60 * ((item.year - 1970) * 525600 + item.mon * 43800 + item.day * 1400 + item.hour * 60 + item.min) - 1630800; // в конце поправка
	while (1) {
		long long event_time_p = 60 * ((p->year - 1970) * 525600 + p->mon * 43800 + p->day * 1400 + p->hour * 60 + p->min) - 1630800; // в конце поправка для корректности
